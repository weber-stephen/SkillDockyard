...
import { Anchor, Bell, Boxes, ClipboardList, FileDown, Inbox, Menu, Send, Settings2, ShieldCheck, X } from "lucide-react";
...
export function AppShell({ children, mode, email, navigation = defaultNavigation }: { children: React.ReactNode; mode: "app" | "demo"; email?: string; navigation?: AppNavigationState }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 hidden w-64 overflow-y-auto border-r border-border bg-panel/85 px-4 py-5 backdrop-blur lg:block"><Brand /><SidebarContent mode={mode} navigation={navigation} /></aside>
    <main className="lg:pl-64"><TopBar mode={mode} email={email} navigation={navigation} onOpenMenu={() => setMenuOpen(true)} /><div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-10">{children}</div></main>
...
function SidebarContent({ mode, navigation, onNavigate }: { mode: "app" | "demo"; navigation: AppNavigationState; onNavigate?: () => void }) {
  ...
  const sections: Array<{ label: string; items: Array<{ href: string; label: string; icon: LucideIcon; count?: number }> }> = [
    { label: "Workspace", items: [{ href: "/", label: "Overview", icon: Boxes }, { href: "/artifacts", label: "Skills", icon: Anchor }] },
    ...
    { label: "Access", items: [{ href: "/invites", label: "Invites", icon: Inbox, count: navigation.pendingInviteCount }] },
    { label: "Administration", items: [{ href: "/settings", label: "Workspace settings", icon: Settings2 }, { href: "/exports", label: "Export inventory", icon: FileDown }] }
  ];
  return <div className="space-y-7"><Link href="/submit" onClick={onNavigate} className="flex h-10 items-center justify-center gap-2 rounded-md bg-primary px-4 text-sm font-semibold text-white transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Send className="h-4 w-4" />Add skill</Link><nav aria-label="Primary navigation" className="space-y-6">{sections.map((section) => <div key={section.label}><h2 className="mb-2 px-3 text-[11px] font-bold uppercase tracking-[0.16em] text-muted-foreground">{section.label}</h2><div className="space-y-1">{section.items.map((item) => <NavigationLink key={item.href} {...item} pathname={pathname} onNavigate={onNavigate} />)}</div></div>)}</nav></div>;
}
