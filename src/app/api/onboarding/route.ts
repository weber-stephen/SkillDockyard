...
export async function GET() {
  ...
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load setup progress." }, { status: 500 }); }
}
...
export async function PUT(request: Request) {
  ...
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to save setup progress." }, { status: 400 }); }
}
